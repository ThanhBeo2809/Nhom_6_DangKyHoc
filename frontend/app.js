...
async function refreshSession() {
  const refreshToken = localStorage.getItem('pka_refresh_token');
  ...
  if (!refreshPromise) {
    refreshPromise = nativeFetch(`${API_BASE}/auth/refresh`, {
      ...
      localStorage.setItem('pka_token', data.token);
      localStorage.setItem('pka_refresh_token', data.refreshToken);
      return true;
    }).finally(() => {
      refreshPromise = null;
    });
...
window.fetch = async function authenticatedFetch(input, init = {}) {
  ...
  const canRefresh = !url.includes('/auth/login') &&
    !url.includes('/auth/refresh') &&
    !url.includes('/auth/forgot-password') &&
    !url.includes('/auth/reset-password');

  // 401 là phiên đăng nhập hết hạn/không hợp lệ. 403 chỉ là thiếu quyền,
  // không được làm mới token hoặc đăng xuất người dùng trong trường hợp đó.
  if (response.status === 401 && canRefresh) {
    const hasRefreshToken = Boolean(localStorage.getItem('pka_refresh_token'));
    if (hasRefreshToken && await refreshSession()) {
      const retryInit = { ...init, headers: new Headers(init.headers || {}) };
      retryInit.headers.set('Authorization', `Bearer ${localStorage.getItem('pka_token')}`);
...
// Đồng bộ trạng thái đăng xuất giữa nhiều tab đang mở cùng lúc.
window.addEventListener('storage', event => {
  if (event.storageArea === localStorage &&
      ['pka_token', 'pka_refresh_token', 'pka_user_id'].includes(event.key) &&
      !localStorage.getItem('pka_token')) {
    handleLogout(false);
  }
});
...
async function handleLoginSubmit(e) {
  ...
  try {
    ...
    if (data.refreshToken) localStorage.setItem('pka_refresh_token', data.refreshToken);
...
async function handleLogout(notifyServer = true) {
  if (notifyServer && localStorage.getItem('pka_token')) {
    try {
      ...
      console.warn('Không thể thông báo đăng xuất tới máy chủ:', error);
    }
  }
  localStorage.removeItem('pka_token');
  localStorage.removeItem('pka_refresh_token');
...
function initSocketIO() {
  ...
  socket = io({ auth: { token: localStorage.getItem('pka_token') } });
  ...
  socket.on('connect', () => {
    ...
    if (state.user?.id) socket.emit('join_user_room', state.user.id);
  });
  ...
  socket.on('notification_created', (notification) => {
    showToast(notification.title || 'Bạn có thông báo mới', 'info');
...
async function loadAcademicContext() {
  try {
    const response = await fetch(`${API_BASE}/auth/context`, { headers: getAuthHeaders() });
    if (response.ok) state.academicContext = await response.json();
  } catch (error) {
    console.warn('Không thể tải cấu hình học kỳ:', error);
  }
}

function currentSemesterId() {
  return state.academicContext?.currentTerm?.id || 'HK1-2026';
}

function currentSemesterName() {
  return state.academicContext?.currentTerm?.name || currentSemesterId();
}
...
async function loadUnreadNotificationCount() {
  ...
  try {
    ...
    console.warn('Không thể tải số thông báo:', error);
  }
}
...
async function loadNotifications(page = 1) {
  ...
  if (!list) return;
  list.innerHTML = '<p class="notification-empty">Đang tải...</p>';
  try {
    const response = await fetch(`${API_BASE}/notifications?page=${page}&limit=8`, { headers: getAuthHeaders() });
    const data = await response.json();
    if (!response.ok) throw new Error(data.message);
    notificationPage = data.pagination.page;
    notificationTotalPages = data.pagination.totalPages;
    document.getElementById('notification-page-label').textContent = `${notificationPage}/${notificationTotalPages}`;
    document.getElementById('notification-prev').disabled = !data.pagination.hasPreviousPage;
    document.getElementById('notification-next').disabled = !data.pagination.hasNextPage;
    list.innerHTML = data.items.length ? data.items.map(item => `
      <div class="notification-item ${item.isRead ? '' : 'unread'}" onclick="markNotificationRead(${item.id}, this)">
        <strong>${escapeHtml(item.title)}</strong>
        <span>${escapeHtml(item.message)}</span>
        <time>${new Date(item.createdAt).toLocaleString('vi-VN')}</time>
      </div>
    `).join('') : '<p class="notification-empty">Bạn chưa có thông báo.</p>';
  } catch (error) {
    list.innerHTML = '<p class="notification-empty">Không thể tải thông báo.</p>';
  }
}
...
function toggleNotificationPanel() {
  ...
  if (!panel.classList.contains('hidden')) loadNotifications(notificationPage);
}
...
function changeNotificationPage(delta) {
  const next = Math.min(Math.max(notificationPage + delta, 1), notificationTotalPages);
  if (next !== notificationPage) loadNotifications(next);
}

function changeAuditPage(page) {
  state.data.auditPage = Math.max(page, 1);
  switchTab(state.currentTab);
}

function getListPage(key) {
  return Math.max(Number(state.data.listPages?.[key] || 1), 1);
}

function changeListPage(key, page) {
  state.data.listPages = { ...(state.data.listPages || {}), [key]: Math.max(Number(page) || 1, 1) };
  switchTab(state.currentTab);
}

function renderListPagination(key, pagination) {
  if (!pagination || pagination.totalPages <= 1) return '';
  return `
    <div class="pagination-controls">
      <button ${pagination.hasPreviousPage ? '' : 'disabled'} onclick="changeListPage('${key}', ${pagination.page - 1})">Trang trước</button>
      <span>Trang ${pagination.page}/${pagination.totalPages} — ${pagination.totalItems} bản ghi</span>
      <button ${pagination.hasNextPage ? '' : 'disabled'} onclick="changeListPage('${key}', ${pagination.page + 1})">Trang sau</button>
    </div>
  `;
}
...
async function renderTabContent(tabId, container) {
  ...
  if (tabId === 'student-dashboard') {
        ...
        <div class="stat-card">
          <div class="stat-info">
            <span class="stat-value text-gradient" style="font-size:1.5rem; margin-top:10px;">
              ${tuitionData.status === 'paid' ? '<span class="badge badge-success">Đã hoàn thành</span>' : `<span class="badge badge-danger">${formatMoney(tuitionData.remainingAmount ?? tuitionData.finalAmount)}</span>`}
            </span>
    ...
    let paymentAction = '';
    if (payment.remainingAmount > 0 && payment.status === 'unpaid') {
      paymentAction = `<button class="btn-primary" onclick="openPaymentModal(${payment.id}, ${payment.remainingAmount})"><i class="fa-solid fa-qrcode"></i> Thanh toán phần còn thiếu bằng mã QR động</button>`;
    ...
    let classRows = '';
    if (payment.enrolledClasses && payment.enrolledClasses.length > 0) {
      payment.enrolledClasses.forEach((c, idx) => {
        classRows += `
          <tr>
            <td>${idx + 1}</td>
            <td><strong>${c.classId}</strong></td>
            <td>${c.courseName} <small style="color:var(--text-secondary)">(${c.courseId})</small></td>
            <td>${c.credits} TC</td>
    ...
    container.innerHTML = `
      <div class="glass-card">
        <h3 class="card-title">Chi tiết các môn học tính phí trong ${escapeHtml(currentSemesterName())} <i class="fa-solid fa-list-check"></i></h3>
        <p style="color:var(--text-secondary); margin-bottom: 15px;">Đơn giá tiêu chuẩn: <strong>${formatMoney(payment.costPerCredit || 1000)} / 1 tín chỉ</strong></p>
        <div class="table-responsive">
          <table class="glass-table">
            ...
            <tbody>
              ${classRows || '<tr><td colspan="6" class="text-center" style="color:var(--text-secondary); padding:20px;">Bạn chưa đăng ký môn học nào trong kỳ này.</td></tr>'}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div class="glass-card" style="margin-top:25px;">
        <h3 class="card-title">Hóa đơn học phí tổng hợp <i class="fa-solid fa-receipt"></i></h3>
        <div class="payment-info-box" style="margin-top:20px;">
          <p><strong>Mã hóa đơn:</strong> PKA-BILL-${payment.id || 'N/A'}</p>
          <p><strong>Tổng số tín chỉ đăng ký:</strong> ${payment.totalCredits || 0} TC</p>
          <p><strong>Tổng tiền gốc:</strong> ${formatMoney(payment.amount || 0)}</p>
          <p><strong>Miễn giảm chính sách / học bổng:</strong> ${(payment.discountRate || 0) * 100}% ${payment.discountReason ? `(${payment.discountReason})` : ''} ${discountAmount > 0 ? `<span style="color:var(--color-warning);">(-${formatMoney(discountAmount)})</span>` : ''}</p>
          <p><strong>Số tiền thực trả:</strong> <span class="text-gradient" style="font-size:1.4rem; font-weight:700;">${formatMoney(payment.finalAmount || 0)}</span></p>
          <p><strong>Đã thanh toán:</strong> <span style="color:var(--color-success); font-weight:700;">${formatMoney(payment.paidAmount || 0)}</span></p>
          <p><strong>Còn phải thanh toán:</strong> <span style="color:var(--color-danger); font-weight:700;">${formatMoney(payment.remainingAmount || 0)}</span></p>
          ${(payment.overpaidAmount || 0) > 0 ? `<p><strong>Số tiền nộp thừa/chờ xử lý:</strong> <span style="color:var(--color-warning); font-weight:700;">${formatMoney(payment.overpaidAmount)}</span></p>` : ''}
          ...
          ${payment.paymentMethod ? `<p><strong>Phương thức nộp:</strong> ${payment.paymentMethod}</p>` : ''}
  ...
  // ==========================================
    ...
    container.innerHTML = `
      <div class="glass-card">
        <h3 class="card-title">Danh sách lớp được phân công giảng dạy - ${escapeHtml(currentSemesterName())} <i class="fa-solid fa-chalkboard-user"></i></h3>
  ...
  // ==========================================
  ...
  else if (tabId === 'pdt-settings') {
    await renderPdtSettingsTab(container);
  }
    ...
    const res = await fetch(`${API_BASE}/pdt/courses?page=${getListPage('pdt-courses')}&limit=20`, { headers });
    const coursePayload = await res.json();
    const courses = coursePayload.items || [];
    ...
    `;
    container.insertAdjacentHTML('beforeend', renderListPagination('pdt-courses', coursePayload.pagination));
  }

  else if (tabId === 'pdt-classes') {
    const res = await fetch(`${API_BASE}/pdt/classes?page=${getListPage('pdt-classes')}&limit=20`, { headers });
    const classPayload = await res.json();
    ...
    `;
    container.insertAdjacentHTML('beforeend', renderListPagination('pdt-classes', classPayload.pagination));
  }

  else if (tabId === 'pdt-tuition') {
    // Quản lý học phí dành cho Phòng Đào Tạo
    const statsRes = await fetch(`${API_BASE}/pdt/tuition/stats`, { headers });
    const stats = await statsRes.json();

    const statusFilter = window.pdtTuitionStatusFilter || '';
    const searchVal = window.pdtTuitionSearch || '';

    let url = `${API_BASE}/pdt/tuition?page=${getListPage('pdt-tuition')}&limit=20`;
    if (statusFilter) url += `&status=${statusFilter}`;
    if (searchVal) url += `&search=${encodeURIComponent(searchVal)}`;

    const listRes = await fetch(url, { headers });
    const paymentPayload = await listRes.json();
    const payments = paymentPayload.items || [];
    ...
    if (Array.isArray(payments)) {
      payments.forEach((p, idx) => {
        ...
        const discountStr = p.discountRate > 0 ? `${p.discountRate * 100}% ${p.discountReason ? '(' + p.discountReason + ')' : ''}` : '0%';

        let actionBtns = `
          <button class="btn-sm btn-secondary" onclick="openPdtDiscountModal(${p.id}, '${p.studentId}', '${(student.name || '').replace(/'/g, "\\'")}', ${p.discountRate || 0}, '${(p.discountReason || '').replace(/'/g, "\\'")}')">
            <i class="fa-solid fa-percent"></i> Miễn giảm
          </button>
        `;

        if (!isPaid) {
          actionBtns += `
            <button class="btn-sm btn-primary" onclick="markPdtManualPayment(${p.id}, '${p.studentId}', '${(student.name || '').replace(/'/g, "\\'")}', ${p.remainingAmount ?? p.finalAmount})">
              <i class="fa-solid fa-money-check-dollar"></i> Gạch nợ
            </button>
          `;
        } else {
          actionBtns += `<span class="badge badge-success" style="margin-left:5px;"><i class="fa-solid fa-check"></i> Đã đóng</span>`;
        }
        ...
        rows += `
          <tr>
            ...
            <td>${discountStr}</td>
            <td><strong style="color:var(--primary); font-size:1.05rem;">${formatMoney(p.finalAmount || 0)}</strong></td>
            <td>${isPaid ? '<span class="badge badge-success">Đã nộp</span>' : '<span class="badge badge-danger">Chưa nộp</span>'}</td>
            <td>${p.transactionId ? `<code>${p.transactionId}</code>` : '-'}</td>
            <td>${actionBtns}</td>
          </tr>
        `;
      });
    ...
    container.innerHTML = `
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-info">
            <span class="stat-value text-gradient">${formatMoney(stats.totalPayable || 0)}</span>
            <span class="stat-label">Tổng phải thu (${escapeHtml(currentSemesterId())})</span>
          </div>
          <i class="fa-solid fa-sack-dollar stat-icon"></i>
        </div>
        <div class="stat-card">
          <div class="stat-info">
            <span class="stat-value text-gradient" style="color:#10b981;">${formatMoney(stats.totalCollected || 0)}</span>
            <span class="stat-label">Đã Thu (${stats.paidCount || 0} SV - ${stats.completionRate || 0}%)</span>
          </div>
          <i class="fa-solid fa-circle-check stat-icon"></i>
        </div>
        <div class="stat-card">
          <div class="stat-info">
            <span class="stat-value text-gradient" style="color:#ef4444;">${formatMoney(stats.totalUnpaid || 0)}</span>
            <span class="stat-label">Còn Nợ (${stats.unpaidCount || 0} SV)</span>
          </div>
          <i class="fa-solid fa-clock stat-icon"></i>
        </div>
        <div class="stat-card">
          <div class="stat-info">
            <span class="stat-value text-gradient">${stats.totalStudents || 0}</span>
            <span class="stat-label">Tổng Sinh Viên Có Hóa Đơn</span>
          </div>
      ...
      <div class="glass-card">
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px; margin-bottom:20px;">
          <h3 class="card-title" style="margin:0;">Danh sách Học phí Sinh viên <i class="fa-solid fa-credit-card"></i></h3>
          <button class="btn-primary" onclick="exportPdtTuitionExcel()"><i class="fa-solid fa-file-excel"></i> Xuất Excel Báo Cáo Học Phí</button>
        </div>

        <!-- Thanh Lọc & Tìm Kiếm -->
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px; margin-bottom:20px;">
          <div style="display:flex; gap:10px; align-items:center;">
            <label style="font-weight:600; font-size:0.9rem;">Trạng thái:</label>
            <select onchange="handlePdtTuitionFilterStatus(this.value)" style="padding:7px 12px; border-radius:8px; border:1px solid rgba(255,255,255,0.2); background:rgba(0,0,0,0.2); color:white;">
              <option value="" ${statusFilter === '' ? 'selected' : ''}>Tất cả trạng thái</option>
              <option value="unpaid" ${statusFilter === 'unpaid' ? 'selected' : ''}>Chưa đóng</option>
              <option value="paid" ${statusFilter === 'paid' ? 'selected' : ''}>Đã đóng</option>
            </select>
          </div>

          <div style="position:relative; width: 300px;">
            <input type="text" placeholder="Tìm theo MSV, Họ tên, Lớp..." value="${escapeHtml(searchVal)}" oninput="handlePdtTuitionSearchInput(this.value)" style="width:100%; padding: 8px 12px 8px 36px; border-radius:8px; border:1px solid rgba(255,255,255,0.2); background:rgba(0,0,0,0.2); color:white; font-size:0.9rem;">
        ...
        <div class="table-responsive">
          <table class="glass-table">
            ...
            <tbody>
              ${rows || '<tr><td colspan="10" class="text-center" style="color:var(--text-secondary); padding:30px;">Không tìm thấy hóa đơn học phí phù hợp.</td></tr>'}
            </tbody>
          </table>
        </div>
      </div>
    `;
    container.insertAdjacentHTML('beforeend', renderListPagination('pdt-tuition', paymentPayload.pagination));
  }

  else if (tabId === 'pdt-warnings') {
    const res = await fetch(`${API_BASE}/pdt/academic-warnings?page=${getListPage('pdt-warnings')}&limit=20`, { headers });
    const warningPayload = await res.json();
    const warnings = warningPayload.items || [];

    let warningRows = '';
    warnings.forEach(w => {
      let actionButtons = '';
      if (w.student.status === 'active') {
        actionButtons = `<button class="btn-sm btn-secondary" onclick="updateStudentWarning('${w.student.id}', 'warning_1')">Áp dụng Cảnh báo Mức 1</button>`;
      } else if (w.student.status === 'warning_1') {
        actionButtons = w.isAtRisk
          ? `
            <button class="btn-sm btn-warning" onclick="updateStudentWarning('${w.student.id}', 'warning_2')">Chuyển Cảnh báo Mức 2</button>
            <button class="btn-sm btn-secondary" style="background:#10b981; border-color:#10b981; color:#fff" onclick="updateStudentWarning('${w.student.id}', 'active')">Khôi phục Bình thường</button>
          `
          : `<button class="btn-sm btn-secondary" style="background:#10b981; border-color:#10b981; color:#fff" onclick="updateStudentWarning('${w.student.id}', 'active')">Xác nhận đã cải thiện</button>`;
      } else if (w.student.status === 'warning_2') {
        actionButtons = `
          <button class="btn-sm btn-secondary" onclick="updateStudentWarning('${w.student.id}', 'warning_1')">Điều chỉnh về Mức 1</button>
          <button class="btn-sm btn-danger" onclick="updateStudentWarning('${w.student.id}', 'dismissed')">Buộc thôi học</button>
          <button class="btn-sm btn-secondary" style="background:#10b981; border-color:#10b981; color:#fff" onclick="updateStudentWarning('${w.student.id}', 'active')">${w.isAtRisk ? 'Khôi phục Bình thường' : 'Xác nhận đã cải thiện'}</button>
        `;
      } else {
        actionButtons = `<button class="btn-sm btn-secondary" style="background:#10b981; border-color:#10b981; color:#fff" onclick="updateStudentWarning('${w.student.id}', 'active')">Khôi phục Bình thường</button>`;
      }
      ...
      warningRows += `
        <tr>
          ...
          <td>
            ...
            <div style="margin-top:6px; font-size:0.78rem; color:${w.isAtRisk ? 'var(--color-danger)' : 'var(--text-success)'};">
              ${w.isAtRisk ? 'Chưa đạt ngưỡng học vụ' : 'Kết quả đã cải thiện'}
            </div>
            ${w.suggestedStatus && w.suggestedStatus !== w.student.status
              ? `<div style="margin-top:6px; font-size:0.78rem; color:var(--text-secondary);">Đề xuất: ${studentStatusLabel(w.suggestedStatus)}</div>`
              : ''}
          </td>
          <td>
            ${actionButtons}
    ...
    container.innerHTML = `
      <div class="glass-card">
        ...
        <p style="color:var(--text-secondary); margin-bottom: 20px;">
          Quy chế cảnh báo: Sinh viên được đưa vào danh sách nếu GPA học kỳ &lt; 1.0 HOẶC CPA tích lũy &lt; 1.5.
          Lần cảnh báo đầu áp dụng Mức 1; nếu kết quả chưa cải thiện ở lần xét tiếp theo thì chuyển Mức 2.
          Cả hai mức được đăng ký tối đa 12 tín chỉ/kỳ. Sinh viên bị buộc thôi học không được đăng ký học phần.
        </p>
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
    ...
    `;
    container.insertAdjacentHTML('beforeend', renderListPagination('pdt-warnings', warningPayload.pagination));
  }

  else if (tabId === 'pdt-logs' || tabId === 'admin-logs') {
    const auditPage = state.data.auditPage || 1;
    const endpointBase = tabId === 'admin-logs' ? `${API_BASE}/admin/audit-logs` : `${API_BASE}/pdt/audit-logs`;
    const endpoint = `${endpointBase}?page=${auditPage}&limit=20`;
    ...
    const logs = logData.items || logData;
    const auditPagination = logData.pagination;
      ...
      <div class="glass-card">
        ...
        ${auditPagination ? `
          <div class="notification-panel-footer">
            <button ${auditPagination.hasPreviousPage ? '' : 'disabled'} onclick="changeAuditPage(${auditPagination.page - 1})">Trang trước</button>
            <span>Trang ${auditPagination.page}/${auditPagination.totalPages} — ${auditPagination.totalItems} bản ghi</span>
            <button ${auditPagination.hasNextPage ? '' : 'disabled'} onclick="changeAuditPage(${auditPagination.page + 1})">Trang sau</button>
          </div>` : ''}
      </div>
    `;
  }
  ...
  // ==========================================
    ...
    const res = await fetch(`${API_BASE}/admin/users?role=student&page=${getListPage('admin-students')}&limit=20`, { headers });
    ...
    `;
    container.insertAdjacentHTML('beforeend', renderListPagination('admin-students', userPayload.pagination));
  }

  else if (tabId === 'admin-lecturers') {
    const res = await fetch(`${API_BASE}/admin/users?role=lecturer&page=${getListPage('admin-lecturers')}&limit=20`, { headers });
    const userPayload = await res.json();
    const userList = userPayload.items || [];
    ...
    `;
    container.insertAdjacentHTML('beforeend', renderListPagination('admin-lecturers', userPayload.pagination));
  }
...
window.handleCourseSearchInput = function(query) {
  ...
  if (tbody) {
    ...
    tbody.innerHTML = listRows || '<tr><td colspan="7" class="text-center" style="color:var(--text-secondary); padding: 30px;">Không tìm thấy học phần nào phù hợp.</td></tr>';
  }
};

async function renderPdtSettingsTab(container) {
  const headers = getAuthHeaders();
  const [termRes, departmentRes, majorRes] = await Promise.all([
    fetch(`${API_BASE}/pdt/academic-terms`, { headers }),
    fetch(`${API_BASE}/pdt/departments`, { headers }),
    fetch(`${API_BASE}/pdt/majors`, { headers })
  ]);
  const [terms, departments, majors] = await Promise.all([
    termRes.json(), departmentRes.json(), majorRes.json()
  ]);
  if (!termRes.ok || !departmentRes.ok || !majorRes.ok) {
    throw new Error('Không thể tải cấu hình đào tạo.');
  }

  const termOptions = terms.map(term => `<option value="${escapeHtml(term.id)}">${escapeHtml(term.name)}</option>`).join('');
  const departmentOptions = departments.map(department => `<option value="${escapeHtml(department.id)}">${escapeHtml(department.name)}</option>`).join('');
  const periods = terms.flatMap(term => (term.RegistrationPeriods || []).map(period => ({ ...period, termName: term.name })));

  container.innerHTML = `
    <div class="settings-grid">
      <section class="glass-card">
        <h3>Học kỳ</h3>
        <form class="inline-form" onsubmit="createAcademicTerm(event)">
          <input id="term-id" required placeholder="Mã học kỳ, ví dụ HK2-2026">
          <input id="term-name" required placeholder="Tên học kỳ">
          <div class="form-row">
            <input id="term-start" type="date" required>
            <input id="term-end" type="date" required>
          </div>
          <label><input id="term-current" type="checkbox"> Đặt làm học kỳ hiện hành</label>
          <button class="btn-primary" type="submit">Thêm học kỳ</button>
        </form>
        <div class="table-responsive"><table class="glass-table">
          <thead><tr><th>Mã</th><th>Tên</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
          <tbody>${terms.map(term => `
            <tr>
              <td>${escapeHtml(term.id)}</td><td>${escapeHtml(term.name)}</td>
              <td>${term.isCurrent ? '<span class="badge badge-success">Hiện hành</span>' : escapeHtml(term.status)}</td>
              <td>
                ${term.isCurrent ? '' : `<button class="btn-sm btn-primary" onclick="setCurrentTerm('${escapeHtml(term.id)}')">Chọn hiện hành</button>`}
                <button class="btn-sm btn-danger" onclick="deletePdtSetting('academic-terms','${escapeHtml(term.id)}')">Xóa</button>
              </td>
            </tr>`).join('')}</tbody>
        </table></div>
      </section>
      ...
      <section class="glass-card">
        <h3>Đợt đăng ký học phần</h3>
        <form class="inline-form" onsubmit="createRegistrationPeriod(event)">
          <select id="period-term" required>${termOptions}</select>
          <input id="period-name" required placeholder="Tên đợt đăng ký">
          <label>Mở cổng<input id="period-start" type="datetime-local" required></label>
          <label>Đóng cổng<input id="period-end" type="datetime-local" required></label>
          <button class="btn-primary" type="submit">Thêm đợt đăng ký</button>
        </form>
        <div class="table-responsive"><table class="glass-table">
          <thead><tr><th>Học kỳ</th><th>Đợt</th><th>Thời gian</th><th>Thao tác</th></tr></thead>
          <tbody>${periods.map(period => `
            <tr>
              <td>${escapeHtml(period.termId)}</td>
              <td>${escapeHtml(period.name)} ${period.isEnabled ? '<span class="badge badge-success">Bật</span>' : '<span class="badge badge-danger">Tắt</span>'}</td>
              <td>${new Date(period.startAt).toLocaleString('vi-VN')}<br>${new Date(period.endAt).toLocaleString('vi-VN')}</td>
              <td>
                <button class="btn-sm btn-secondary" onclick="toggleRegistrationPeriod(${period.id}, ${!period.isEnabled})">${period.isEnabled ? 'Tắt' : 'Bật'}</button>
                <button class="btn-sm btn-danger" onclick="deletePdtSetting('registration-periods',${period.id})">Xóa</button>
              </td>
            </tr>`).join('')}</tbody>
        </table></div>
      </section>
      ...
      <section class="glass-card">
        <h3>Khoa</h3>
        <form class="inline-form" onsubmit="createDepartment(event)">
          <input id="department-id" required placeholder="Mã khoa">
          <input id="department-name" required placeholder="Tên khoa">
          <button class="btn-primary" type="submit">Thêm khoa</button>
        </form>
        <div class="table-responsive"><table class="glass-table">
          <thead><tr><th>Mã</th><th>Tên</th><th>Thao tác</th></tr></thead>
          <tbody>${departments.map(department => `
            <tr><td>${escapeHtml(department.id)}</td><td>${escapeHtml(department.name)}</td><td>
              <button class="btn-sm btn-secondary" onclick="renamePdtSetting('departments','${escapeHtml(department.id)}','${escapeHtml(department.name)}')">Sửa</button>
              <button class="btn-sm btn-danger" onclick="deletePdtSetting('departments','${escapeHtml(department.id)}')">Xóa</button>
            </td></tr>`).join('')}</tbody>
        </table></div>
      </section>
      ...
      <section class="glass-card">
        <h3>Ngành đào tạo</h3>
        <form class="inline-form" onsubmit="createMajor(event)">
          <input id="major-id" required placeholder="Mã ngành">
          <input id="major-name" required placeholder="Tên ngành">
          <select id="major-department" required>${departmentOptions}</select>
          <button class="btn-primary" type="submit">Thêm ngành</button>
        </form>
        <div class="table-responsive"><table class="glass-table">
          <thead><tr><th>Mã</th><th>Tên</th><th>Khoa</th><th>Thao tác</th></tr></thead>
          <tbody>${majors.map(major => `
            <tr><td>${escapeHtml(major.id)}</td><td>${escapeHtml(major.name)}</td><td>${escapeHtml(major.Department?.name || major.departmentId)}</td><td>
              <button class="btn-sm btn-secondary" onclick="renamePdtSetting('majors','${escapeHtml(major.id)}','${escapeHtml(major.name)}')">Sửa</button>
              <button class="btn-sm btn-danger" onclick="deletePdtSetting('majors','${escapeHtml(major.id)}')">Xóa</button>
            </td></tr>`).join('')}</tbody>
        </table></div>
      </section>
    </div>`;
...
async function submitPdtSetting(path, method, body) {
  const response = await fetch(`${API_BASE}/pdt/${path}`, {
    method,
    headers: getAuthHeaders(),
    body: JSON.stringify(body)
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || 'Không thể cập nhật cấu hình.');
  showToast(data.message || 'Cập nhật thành công.', 'success');
  await loadAcademicContext();
  switchTab('pdt-settings');
}

async function createAcademicTerm(event) {
  event.preventDefault();
  try {
    await submitPdtSetting('academic-terms', 'POST', {
      id: document.getElementById('term-id').value.trim(),
      ...
      status: document.getElementById('term-current').checked ? 'active' : 'planned',
      isCurrent: document.getElementById('term-current').checked
    });
  } catch (error) { showToast(error.message, 'error'); }
}

async function setCurrentTerm(id) {
  try { await submitPdtSetting(`academic-terms/${encodeURIComponent(id)}`, 'PUT', { isCurrent: true, status: 'active' }); }
  catch (error) { showToast(error.message, 'error'); }
}

async function createRegistrationPeriod(event) {
  event.preventDefault();
  try {
    await submitPdtSetting('registration-periods', 'POST', {
      termId: document.getElementById('period-term').value,
      name: document.getElementById('period-name').value.trim(),
      startAt: document.getElementById('period-start').value,
      endAt: document.getElementById('period-end').value,
      isEnabled: true
    });
  } catch (error) { showToast(error.message, 'error'); }
}

async function toggleRegistrationPeriod(id, isEnabled) {
  try { await submitPdtSetting(`registration-periods/${id}`, 'PUT', { isEnabled }); }
  catch (error) { showToast(error.message, 'error'); }
}
...
async function createDepartment(event) {
  event.preventDefault();
  try {
    await submitPdtSetting('departments', 'POST', {
...
async function createMajor(event) {
  event.preventDefault();
  try {
    await submitPdtSetting('majors', 'POST', {
      id: document.getElementById('major-id').value.trim(),
      name: document.getElementById('major-name').value.trim(),
      departmentId: document.getElementById('major-department').value
    });
  } catch (error) { showToast(error.message, 'error'); }
}

async function renamePdtSetting(path, id, currentName) {
  const name = prompt('Nhập tên mới:', currentName);
  if (!name || name.trim() === currentName) return;
  try { await submitPdtSetting(`${path}/${encodeURIComponent(id)}`, 'PUT', { name: name.trim() }); }
  catch (error) { showToast(error.message, 'error'); }
}

async function deletePdtSetting(path, id) {
  if (!confirm('Bạn chắc chắn muốn xóa dữ liệu này?')) return;
  try { await submitPdtSetting(`${path}/${encodeURIComponent(id)}`, 'DELETE', {}); }
  catch (error) { showToast(error.message, 'error'); }
}
...
async function renderStudentRegisterTab(container, scope = 'my_major', searchQuery = '') {
  ...
  try {
    const [res, statusRes] = await Promise.all([
      fetch(`${API_BASE}/student/courses/available?scope=${scope}`, { headers }),
      fetch(`${API_BASE}/student/registration-status`, { headers })
    ]);
    const classStats = await res.json();
    const registrationStatus = await statusRes.json();

    if (!res.ok) {
      throw new Error(classStats.message || `Không thể tải danh sách lớp học phần (HTTP ${res.status}).`);
    }

    if (!Array.isArray(classStats)) {
      throw new Error('Dữ liệu danh sách lớp học phần không đúng định dạng.');
    }

    state.data.registrationStatus = statusRes.ok ? registrationStatus : { isOpen: false };
    ...
    container.innerHTML = `
      <div class="glass-card text-center">
        <p style="color:var(--color-danger);">${escapeHtml(err.message || 'Có lỗi xảy ra khi kết nối máy chủ.')}</p>
        <button class="btn-secondary" style="margin-top:12px;" onclick="switchTab('student-register')">
...
function generateStudentRegisterRowsHTML(classStats, searchQuery = '') {
  ...
  if (normalizedQuery) {
    filtered = classStats.filter(item => {
      ...
      return courseName.includes(normalizedQuery) ||
        courseId.includes(normalizedQuery) ||
        classId.includes(normalizedQuery) ||
        lecturerName.includes(normalizedQuery) ||
        roomName.includes(normalizedQuery);
    });
  }

  const priorityByType = { retake: 1, improve: 2, new: 3, regular: 3 };
  filtered = [...filtered].sort((left, right) => {
    const leftPriority = left.registrationPriority
      ?? priorityByType[left.regType]
      ?? 3;
    const rightPriority = right.registrationPriority
      ?? priorityByType[right.regType]
      ?? 3;
    if (leftPriority !== rightPriority) return leftPriority - rightPriority;

    const leftCourseId = left.classInfo.Course?.id || left.classInfo.courseId || '';
    const rightCourseId = right.classInfo.Course?.id || right.classInfo.courseId || '';
    const courseDifference = String(leftCourseId).localeCompare(
      String(rightCourseId),
      'vi'
    );
  ...
  filtered.forEach(item => {
    const courseId = item.classInfo.Course?.id || item.classInfo.courseId;
    if (!courseGroups.has(courseId)) courseGroups.set(courseId, []);
    courseGroups.get(courseId).push(item);
  });

  let listRows = '';
  courseGroups.forEach((items, courseId) => {
    const firstItem = items[0];
    const course = firstItem.classInfo.Course;
    const groupKey = encodeURIComponent(courseId);
    const isExpanded = normalizedQuery || window.expandedCourseGroups?.has(groupKey);

    let subjectBadge = '';
    if (firstItem.isGeneral) {
      subjectBadge = `<span class="badge badge-info" style="font-weight:600;"><i class="fa-solid fa-book-open"></i> Đại cương</span>`;
    } else if (firstItem.courseMajorName && firstItem.courseMajorName.includes('Cơ sở ngành')) {
      subjectBadge = `<span class="badge badge-primary" style="font-weight:600;"><i class="fa-solid fa-layer-group"></i> ${escapeHtml(firstItem.courseMajorName)}</span>`;
    } else {
      subjectBadge = `<span class="badge badge-success" style="font-weight:600;"><i class="fa-solid fa-graduation-cap"></i> ${escapeHtml(firstItem.courseMajorName || 'Chuyên ngành')}</span>`;
    }

    let regTypeBadge = '';
    if (firstItem.regType === 'retake') {
      regTypeBadge = `<span class="badge badge-danger" style="font-weight:600;"><i class="fa-solid fa-1"></i> Ưu tiên 1 · Học lại (Thi trượt F)</span>`;
    } else if (firstItem.regType === 'improve') {
      regTypeBadge = `<span class="badge badge-warning" style="font-weight:600;"><i class="fa-solid fa-2"></i> Ưu tiên 2 · Học nâng điểm</span>`;
    } else {
      regTypeBadge = `<span class="badge badge-secondary" style="font-weight:500;"><i class="fa-solid fa-3"></i> Ưu tiên 3 · Học mới</span>`;
    }

    let prereqDisplay = '<span style="color:var(--text-secondary)">Không có</span>';
    if (firstItem.prereqInfo) {
      if (firstItem.prereqInfo.isPassed) {
        prereqDisplay = `<span style="color:#10b981; font-weight:600; font-size:0.85rem;"><i class="fa-solid fa-circle-check"></i> Đã đạt (${escapeHtml(firstItem.prereqInfo.requiredId)})</span>`;
      } else {
        prereqDisplay = `<span style="color:#f87171; font-weight:600; font-size:0.85rem;" title="Yêu cầu phải đạt môn tiên quyết: ${escapeHtml(firstItem.prereqInfo.requiredName)}"><i class="fa-solid fa-lock"></i> Cần môn ${escapeHtml(firstItem.prereqInfo.requiredId)}</span>`;
      }
    }

    const selectedItem = items.find(item => item.myStatus);
    let groupStatus = `<span class="badge badge-secondary">${items.length} lớp đang mở</span>`;
    if (selectedItem?.myStatus === 'enrolled') {
      groupStatus = `<span class="badge badge-success"><i class="fa-solid fa-circle-check"></i>&nbsp; Đã chọn ${escapeHtml(selectedItem.classInfo.id)}</span>`;
    } else if (selectedItem?.myStatus === 'waitlist') {
      groupStatus = `<span class="badge badge-warning"><i class="fa-solid fa-clock"></i>&nbsp; Đang chờ ${escapeHtml(selectedItem.classInfo.id)}</span>`;
    } else if (firstItem.registeredClassId) {
      groupStatus = `<span class="badge badge-success"><i class="fa-solid fa-circle-check"></i>&nbsp; Đã chọn ${escapeHtml(firstItem.registeredClassId)}</span>`;
    }

    const classRows = items.map(item => {
      const cls = item.classInfo;
      ...
      const registrationOpen = state.data.registrationStatus?.isOpen === true;
      let actionBtn = '';

      if (item.myStatus === 'enrolled') {
        actionBtn = `<button class="btn-sm btn-danger" ${registrationOpen ? `onclick="unregisterClass('${cls.id}')"` : 'disabled title="Cổng đăng ký đang đóng"'}><i class="fa-solid fa-trash"></i> Hủy lớp</button>`;
      } else if (item.myStatus === 'waitlist') {
        actionBtn = `
          <span class="badge badge-warning"><i class="fa-solid fa-spinner fa-spin"></i> Hàng chờ</span>
          <button class="btn-sm btn-danger" ${registrationOpen ? `onclick="unregisterClass('${cls.id}')"` : 'disabled title="Cổng đăng ký đang đóng"'}><i class="fa-solid fa-xmark"></i> Hủy</button>
        `;
      } else if (item.registeredClassId) {
        const registeredState = item.registeredStatus === 'waitlist' ? 'Đang chờ' : 'Đã chọn';
        actionBtn = `<button class="btn-sm btn-secondary" disabled title="Mỗi môn chỉ được chọn một lớp trong cùng học kỳ"><i class="fa-solid fa-ban"></i> ${registeredState} ${escapeHtml(item.registeredClassId)}</button>`;
      } else {
        ...
        actionBtn = `<button class="btn-sm ${btnClass}" ${registrationOpen ? `onclick="registerClass('${cls.id}')"` : 'disabled title="Cổng đăng ký đang đóng"'}>${btnText}</button>`;
      }

      return `
        <tr>
          <td><strong>${escapeHtml(cls.id)}</strong></td>
          <td>${escapeHtml(cls.Lecturer?.name || 'Chưa phân công')}</td>
          <td>${dayStr} (${shiftStr})<br><small>${slotStr}</small></td>
          <td>${escapeHtml(cls.roomName || '-')}</td>
          <td>
            <strong>${item.enrolledCount}/${cls.capacity}</strong>
            ${item.waitlistCount > 0 ? `<br><small class="waitlist-text">Chờ: ${item.waitlistCount}</small>` : ''}
          </td>
          <td class="course-class-action">${actionBtn}</td>
        </tr>
      `;
    }).join('');

    listRows += `
      <tr class="course-group-row ${isExpanded ? 'expanded' : ''}"
          id="course-group-${groupKey}"
          onclick="toggleCourseClassGroup('${groupKey}')"
          role="button"
          tabindex="0"
          aria-expanded="${isExpanded ? 'true' : 'false'}"
          onkeydown="if(event.key === 'Enter' || event.key === ' '){ event.preventDefault(); toggleCourseClassGroup('${groupKey}'); }">
        <td>
          <span class="course-expand-icon"><i class="fa-solid fa-chevron-right"></i></span>
          <strong>${escapeHtml(courseId)}</strong>
        </td>
        <td>
          <div class="course-group-name">${escapeHtml(course?.name || '')}</div>
          <small>${items.length} lớp học phần</small>
        </td>
        <td>${subjectBadge}</td>
        <td><span class="badge badge-secondary"><i class="fa-solid fa-clock-rotate-left"></i>&nbsp; Kỳ ${firstItem.term || 1}</span></td>
        <td>${prereqDisplay}</td>
        <td>
          <strong>${course?.credits || 0} TC</strong>
          <div class="course-purpose">${regTypeBadge}</div>
        </td>
        <td>
          ${groupStatus}
          <div class="course-expand-hint">Nhấn để ${isExpanded ? 'thu gọn' : 'xem các lớp'}</div>
        </td>
      </tr>
      <tr class="course-group-detail" id="course-classes-${groupKey}" ${isExpanded ? '' : 'hidden'}>
        <td colspan="7">
          <div class="course-class-list">
            <table class="course-class-table">
              <thead>
                <tr>
                  <th>Mã lớp</th>
                  <th>Giảng viên</th>
...
window.expandedCourseGroups = window.expandedCourseGroups || new Set();
window.toggleCourseClassGroup = function(groupKey) {
  const summaryRow = document.getElementById(`course-group-${groupKey}`);
  const detailRow = document.getElementById(`course-classes-${groupKey}`);
  if (!summaryRow || !detailRow) return;

  const willExpand = detailRow.hidden;
  detailRow.hidden = !willExpand;
  summaryRow.classList.toggle('expanded', willExpand);
  summaryRow.setAttribute('aria-expanded', String(willExpand));

  const hint = summaryRow.querySelector('.course-expand-hint');
  if (hint) hint.textContent = willExpand ? 'Nhấn để thu gọn' : 'Nhấn để xem các lớp';

  if (willExpand) {
    window.expandedCourseGroups.add(groupKey);
  } else {
    window.expandedCourseGroups.delete(groupKey);
  }
};
...
function renderStudentRegisterHTML(container, classStats, scope, searchQuery = '') {
  ...
  const progressText = academicProgress ? `${academicProgress.yearText} - ${academicProgress.semesterText}` : currentSemesterName();
  const registrationStatus = state.data.registrationStatus || {};
  const period = registrationStatus.period;
  const statusMessage = registrationStatus.isOpen
    ? `Cổng đang mở${period ? ` đến ${new Date(period.endAt).toLocaleString('vi-VN')}` : ''}.`
    : `Cổng hiện đang đóng${registrationStatus.nextPeriod ? `; đợt tiếp theo mở lúc ${new Date(registrationStatus.nextPeriod.startAt).toLocaleString('vi-VN')}` : '.'}`;

  container.innerHTML = `
    <div class="registration-status-banner ${registrationStatus.isOpen ? 'open' : 'closed'}">
      <strong><i class="fa-solid ${registrationStatus.isOpen ? 'fa-lock-open' : 'fa-lock'}"></i> ${escapeHtml(statusMessage)}</strong>
    </div>
    <div class="glass-card">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px; margin-bottom:15px;">
        <div>
          ...
          <p style="color:var(--text-secondary); margin: 5px 0 0 0; font-size:0.9rem;">
            Lưu ý: Hệ thống chỉ hiển thị các môn học thuộc đúng lộ trình <strong>${escapeHtml(progressText)}</strong> của Ngành bạn đang học và các môn cần Học lại. Nhấn vào một học phần để xem các lớp đang mở.
          </p>
          <p style="color:var(--text-secondary); margin: 5px 0 0 0; font-size:0.9rem;">
            Thứ tự ưu tiên đăng ký: <strong style="color:#f87171;">1. Học lại</strong>
            → <strong style="color:#fbbf24;">2. Nâng điểm</strong>
            → <strong>3. Học mới theo chương trình đào tạo</strong>.
          </p>
        </div>
      </div>
      ...
      <div style="display:flex; justify-content:flex-end; align-items:center; margin-bottom:20px;">
        <div style="position:relative; width: 320px;">
          <input type="text" id="course-search-input" placeholder="Tìm học phần, mã lớp, giảng viên..." value="${escapeHtml(searchQuery)}" oninput="handleCourseSearchInput(this.value)" style="width:100%; padding: 8px 12px 8px 36px; border-radius:8px; border:1px solid rgba(255,255,255,0.2); background:rgba(0,0,0,0.2); color:white; font-size:0.9rem;">
      ...
      <div class="table-responsive">
        <table class="glass-table">
          ...
          <tbody id="student-register-tbody">
            ${listRows || '<tr><td colspan="7" class="text-center" style="color:var(--text-secondary); padding: 30px;">Không tìm thấy học phần nào phù hợp.</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>
...
async function openClassGrading(classId) {
  ...
  section.innerHTML = `
    <div class="glass-card" style="margin-top:30px; animation: floatIn 0.4s ease;">
      <h3 class="card-title">
        Nhập điểm lớp học phần: ${classId}
        <button class="btn-sm btn-primary" onclick="exportClassGradesExcel('${classId}')" style="margin-left:10px;"><i class="fa-solid fa-file-excel"></i> Xuất Excel</button>
        <button class="btn-sm btn-secondary" onclick="document.getElementById('grade-import-file').click()"><i class="fa-solid fa-file-import"></i> Nhập Excel</button>
        <input id="grade-import-file" type="file" accept=".xlsx" class="hidden" onchange="importClassGradesExcel('${classId}', this.files[0])">
...
async function importClassGradesExcel(classId, file) {
  if (!file) return;
  if (!file.name.toLowerCase().endsWith('.xlsx')) {
    showToast('Chỉ chấp nhận tệp Excel .xlsx.', 'error');
    return;
  }
  if (file.size > 8 * 1024 * 1024) {
    showToast('Tệp Excel không được vượt quá 8 MB.', 'error');
    return;
  }
  try {
    ...
    const response = await fetch(`${API_BASE}/lecturer/classes/${classId}/import-excel`, {
      method: 'POST',
      headers: getAuthHeaders(),
    ...
    const data = await response.json();
    if (!response.ok) {
      const details = Array.isArray(data.errors) ? ` ${data.errors.slice(0, 3).join(' ')}` : '';
      throw new Error((data.message || 'Không thể nhập điểm.') + details);
    }
    showToast(data.message, 'success');
    openClassGrading(classId);
  } catch (error) {
    showToast(error.message || 'Không thể đọc tệp Excel.', 'error');
  }
}
...
async function createClass(e) {
  ...
  try {
    const res = await fetch(`${API_BASE}/pdt/classes`, {
      ...
      headers: getAuthHeaders(),
      body: JSON.stringify({
        id, courseId, lecturerId, roomName, roomType, capacity, semester: currentSemesterId(), dayOfWeek, shift, startSlot, numSlots
      })
    });
    const data = await res.json();
    if (res.ok) {
...
function studentStatusLabel(status) {
  switch (status) {
    ...
    case 'warning_1': return 'Cảnh báo Mức 1';
    case 'warning_2': return 'Cảnh báo Mức 2';
    case 'dismissed': return 'Buộc thôi học';
...
async function handleRequestOTP(e) {
  ...
  try {
    ...
    forgotPasswordState.otpCode = data.otp || '';
    ...
    if (data.otp) {
      document.getElementById('current-otp-val').textContent = data.otp;
      document.getElementById('current-otp-badge').classList.remove('hidden');
      document.getElementById('forgot-otp').value = data.otp;
    } else {
      document.getElementById('current-otp-badge').classList.add('hidden');
...
window.exportPdtTuitionExcel = function() {
  downloadFile('/pdt/tuition/export-excel', 'Danh_Sach_Hoc_Phi.xlsx');
};

// PDT Lọc & Tìm kiếm Học phí
window.handlePdtTuitionFilterStatus = function(val) {
  window.pdtTuitionStatusFilter = val;
  state.data.listPages = { ...(state.data.listPages || {}), 'pdt-tuition': 1 };
  switchTab(state.currentTab);
};

window.handlePdtTuitionSearchInput = function(val) {
  window.pdtTuitionSearch = val;
  state.data.listPages = { ...(state.data.listPages || {}), 'pdt-tuition': 1 };
  if (window.pdtTuitionSearchTimeout) clearTimeout(window.pdtTuitionSearchTimeout);
  window.pdtTuitionSearchTimeout = setTimeout(() => {
    switchTab(state.currentTab);
  }, 300);
};

// Modal Miễn giảm học phí cho PDT
function openPdtDiscountModal(paymentId, studentId, studentName, currentRate, currentReason) {
  const modal = document.getElementById('pdt-discount-modal');
  if (!modal) {
    const newRateStr = prompt(`Nhập tỷ lệ miễn giảm mới cho SV ${studentName} (${studentId}) (0.0 -> 1.0, vd 0.5 là giảm 50%):`, currentRate);
    if (newRateStr === null) return;
    const rate = parseFloat(newRateStr);
    if (isNaN(rate) || rate < 0 || rate > 1) {
      alert('Tỷ lệ không hợp lệ! Nhập số từ 0.0 đến 1.0.');
      return;
    }
    const reason = prompt('Nhập lý do miễn giảm / tên học bổng:', currentReason || '');
    submitPdtDiscountApi(paymentId, rate, reason);
    return;
  }

  document.getElementById('discount-payment-id').value = paymentId;
  document.getElementById('discount-student-info').textContent = `${studentName} (${studentId})`;
  document.getElementById('discount-rate-select').value = String(currentRate);
  document.getElementById('discount-reason-input').value = currentReason || '';

  modal.classList.remove('hidden');
}
window.openPdtDiscountModal = openPdtDiscountModal;

function closePdtDiscountModal() {
  const modal = document.getElementById('pdt-discount-modal');
  if (modal) modal.classList.add('hidden');
}
window.closePdtDiscountModal = closePdtDiscountModal;

async function submitPdtDiscountApi(paymentId, discountRate, discountReason) {
  try {
    const res = await fetch(`${API_BASE}/pdt/tuition/${paymentId}/discount`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ discountRate, discountReason })
    });
    const data = await res.json();
    alert(data.message || 'Cập nhật miễn giảm thành công!');
    if (res.ok) {
      closePdtDiscountModal();
      switchTab(state.currentTab);
    }
  } catch (err) {
    console.error(err);
    alert('Lỗi cập nhật miễn giảm học phí.');
  }
}

async function handleSavePdtDiscount(e) {
  e.preventDefault();
  const paymentId = document.getElementById('discount-payment-id').value;
  const rate = parseFloat(document.getElementById('discount-rate-select').value);
  const reason = document.getElementById('discount-reason-input').value.trim();

  await submitPdtDiscountApi(paymentId, rate, reason);
}
window.handleSavePdtDiscount = handleSavePdtDiscount;

// PDT Gạch nợ thủ công
async function markPdtManualPayment(paymentId, studentId, studentName, finalAmount) {
  const notes = prompt(`Xác nhận gạch nợ trực tiếp (Tiền mặt / Chuyển khoản tại quầy) cho SV ${studentName} (${studentId}) - Số tiền: ${formatMoney(finalAmount)}\nNhập ghi chú gạch nợ (nếu có):`, 'Đã thu tiền mặt tại Phòng Đào Tạo');
  if (notes === null) return;

  try {
    const res = await fetch(`${API_BASE}/pdt/tuition/${paymentId}/manual-pay`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ method: 'Tiền mặt / Trực tiếp PDT', notes })
    });
    const data = await res.json();
    alert(data.message || 'Gạch nợ học phí thành công!');
    if (res.ok) {
      switchTab(state.currentTab);
    }
  } catch (err) {
    console.error(err);
    alert('Lỗi xác nhận gạch nợ thủ công.');
  }
}
window.markPdtManualPayment = markPdtManualPayment;
...
// ==========================================================================
// THỜI KHÓA BIỂU: LỊCH CÁ NHÂN TRONG PHẠM VI HỌC KỲ
// ==========================================================================

function parseAcademicDate(dateValue) {
  if (!dateValue) return null;
  const parsed = new Date(`${dateValue}T00:00:00`);
  return Number.isNaN(parsed.getTime()) ? null : parsed;
}

function getScheduleTermRange() {
  const term = state.academicContext?.currentTerm;
  const startDate = parseAcademicDate(term?.startDate);
  const endDate = parseAcademicDate(term?.endDate);
  if (!startDate || !endDate) return null;
  endDate.setHours(23, 59, 59, 999);
  return { startDate, endDate };
}

function isDateWithinScheduleTerm(date, termRange = getScheduleTermRange()) {
  if (!termRange) return true;
  const value = new Date(date);
  return value >= termRange.startDate && value <= termRange.endDate;
}
...
function getClassTimeRange(schedule) {
  const startSlot = Number(schedule.startSlot);
  const numSlots = Math.max(1, Number(schedule.numSlots) || 1);
  const endSlot = startSlot + numSlots - 1;
  const startTime = CLASS_SLOT_TIMES[startSlot]?.start;
  const endTime = CLASS_SLOT_TIMES[endSlot]?.end;
  ...
  return {
    startTime,
    endTime,
    startMinutes: timeTextToMinutes(startTime),
    endMinutes: timeTextToMinutes(endTime)
  };
}
...
async function renderStudentScheduleTab(container, scheduleMode = 'student') {
  try {
    ...
    const displayedCalDate = new Date(window.miniCalDisplayedDate || selectedDate);
    window.selectedScheduleDate = new Date(selectedDate);
    window.miniCalDisplayedDate = new Date(displayedCalDate);
    ...
    // Header các ngày trong tuần. Mỗi phần tử được đặt vào đúng tọa độ
    // để các thẻ lịch có thể phủ lên nhiều hàng giờ mà không đẩy lệch lưới.
    let dayHeadersHTML = `<div class="grid-time-header" style="grid-column:1; grid-row:1;">🕒<br>Giờ VN</div>`;
    weekDays.forEach((wd, dayIndex) => {
      ...
      dayHeadersHTML += `
        <div class="grid-time-header" style="grid-column:${dayIndex + 2}; grid-row:1; ${highlightBg}">
    ...
    // Vẽ nền lưới giờ cố định, không chèn thẻ môn học vào trong một ô giờ.
    let gridRowsHTML = '';
    hours.forEach((hr, hourIndex) => {
      const gridRow = hourIndex + 2;
      gridRowsHTML += `<div class="grid-time-slot-label" style="grid-column:1; grid-row:${gridRow};">${hr.label}</div>`;
      weekDays.forEach((_wd, dayIndex) => {
        gridRowsHTML += `<div class="grid-hourly-cell" style="grid-column:${dayIndex + 2}; grid-row:${gridRow};"></div>`;
      });
    });

    // Đặt thẻ lịch theo số phút thực tế. Ví dụ 06:45–09:25 bắt đầu
    // tại 75% hàng 6:00 và kéo dài qua các hàng 7:00, 8:00, 9:00.
    const gridStartMinutes = 6 * 60;
    const gridHourHeight = 64;
    let scheduleCardsHTML = '';
    schedules.forEach(c => {
      const dayIndex = weekDays.findIndex(wd => wd.dayOfWeek === Number(c.dayOfWeek));
      const timeRange = getClassTimeRange(c);
      if (dayIndex < 0 || !timeRange) return;
      if (!isDateWithinScheduleTerm(weekDays[dayIndex].dateObj, termRange)) return;

      const startOffsetMinutes = timeRange.startMinutes - gridStartMinutes;
      const durationMinutes = timeRange.endMinutes - timeRange.startMinutes;
      if (startOffsetMinutes < 0 || durationMinutes <= 0) return;

      const startHourIndex = Math.floor(startOffsetMinutes / 60);
      const offsetWithinHour = startOffsetMinutes % 60;
      const gridRow = startHourIndex + 2;
      const coveredRows = Math.max(1, Math.ceil((offsetWithinHour + durationMinutes) / 60));
    ...
    const termRangeText = termRange
      ? `${formatScheduleDate(termRange.startDate)} – ${formatScheduleDate(termRange.endDate)}`
      : 'Chưa cấu hình thời gian học kỳ';
